...
export default {
  ...
  data() {
    return {
      ...
      cssSelectedText:
        'Selected flex items-center justify-center px-4 h-10 leading-tight  bg-gray-400 hover:bg-gray-500 cursor-pointer',
      currentPage: 1
    }
  },
  methods: {
    ...
    <ul class="flex items-center-space-x-px h-10 text-base">
      <li v-for="page in totalPage" :key="page">
        <p
          ...
          :class="[
            page == currentPage ? cssSelectedText : cssText,
            page === 1 ? 'rounded-l-md' : '',
            page === totalPage ? 'rounded-r-md' : ''
          ]"
        >
          {{ page }}
        </p>
